            ...
            <table class="table table-bordered table-striped {{ count($invitations) > 0 ? 'datatable' : '' }} @can('invitation_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        ...
                        <th>@lang('global.invitations.fields.fullname')</th>
                                ...
                                <td>
                                    @can('invitation_view')
                                        @if (!$invitation->sent_at)
                                            <a href="{{ route('admin.invitations.send',[$invitation->id]) }}" class="btn btn-xs btn-primary"><i class="fa fa-paper-plane" aria-hidden="true"></i></a>
                                        @endif
                                    @endcan
                                    @can('invitation_edit')
                                    <a href="{{ route('admin.invitations.edit',[$invitation->id]) }}" class="btn btn-xs btn-info"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    @endcan
                                    @can('invitation_delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
